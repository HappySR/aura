<script lang="ts">
  ...
  // Active flood zone data
  const activeFloodZones: FloodZone[] = [
    {
  ...
  async function renderMap() {
    ...
    try {
      ...
      // Add flood markers
      floodLocations.forEach((flood) => {
        // @ts-ignore
        const [x, y] = projection([
          currentLocation.coordinates.lng + (flood.x - 0.5) * 2,
          currentLocation.coordinates.lat + (flood.y - 0.5) * 2
        ]);

        const gradient = svg
          .append("defs")
          .append("radialGradient")
          .attr("id", `flood-gradient-${flood.x}-${flood.y}`);

        gradient.append("stop").attr("offset", "0%").attr("stop-color", "rgba(0, 0, 255, 1)");

        gradient.append("stop").attr("offset", "100%").attr("stop-color", "rgba(135, 206, 235, 0)");

        svg
          .append("circle")
          .attr("cx", x)
          .attr("cy", y)
          .attr("r", flood.intensity * 20)
          .attr("fill", `url(#flood-gradient-${flood.x}-${flood.y})`);
      });
    } catch (error) {
  ...
  <div class="mx-auto max-w-7xl space-y-8">
    ...
    <div class="mt-20 flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div class="w-full space-y-6 lg:w-1/2">
        <div class="space-y-6">
          <h1
            class="bg-gradient-to-r from-blue-500 via-sky-500 to-blue-500 bg-clip-text text-center text-4xl font-bold tracking-tight text-transparent md:text-left md:text-5xl lg:text-6xl"
          >
            Flood Monitor
          </h1>

          <div
            class="group rounded-xl border border-border/50 bg-background/20 p-6 backdrop-blur-sm transition-all hover:border-blue-500/50 hover:bg-sky-500/10"
          >
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-2xl font-bold text-blue-500">{currentLocation.region}</p>
          ...
          >
            <div class="flex h-full items-center justify-center gap-3">
              ...
              <div>
                ...
                <p class="font-medium text-blue-500">Critical</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="space-y-4 sm:space-y-6">
      <h3 class="text-xl font-semibold sm:text-2xl">Active Flood Incidents</h3>
      <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
        {#each activeFloodZones as zone}
          <div
            class="group relative rounded-xl border border-border/50 bg-background/50 p-4 backdrop-blur-sm transition-all duration-300 hover:border-blue-500/50 sm:p-6"
          >
            <div
            ...
            <div class="relative space-y-3 sm:space-y-4">
              ...
              <button
                class="inline-flex items-center gap-2 text-xs text-blue-500 transition-colors hover:text-blue-400 sm:text-sm"
              >
    ...
    <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-4">
      ...
      >
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          <Badge variant="outline" class="flex w-fit items-center gap-2">
            <Clock class="h-3 w-3" />
            Updated at
            <span class="font-medium text-blue-500">{environmentalData.lastUpdated}</span>
          </Badge>
        </p>
      </div>
      ...
      >
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          <Badge variant="outline" class="flex w-fit items-center gap-2">
            <Clock class="h-3 w-3" />
            Synced at <span class="font-medium text-blue-500">{iotData.lastSync}</span>
          </Badge>
        </p>
      </div>
